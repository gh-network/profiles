...
namespace GhostNetwork.Profiles.MongoDb
{
    public class MongoFriendsFuntionalityStorage : IFriendsFuntionalityStorage
    {
        ...
        public async Task<Friend> GetExistFriendRequestAsync(Guid fromUserId, Guid toUserId)
        {
            var filter = Builders<FriendEntity>.Filter.Eq(p => p.FromUser, fromUserId)
                & Builders<FriendEntity>.Filter.Eq(p => p.ToUser, toUserId);

            var result = await context.Friends.Find(filter).FirstOrDefaultAsync();
